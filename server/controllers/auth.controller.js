...
module.exports.getHouses = async (req, res) => {
 ...
 module.exports.findHouses = async (req, res) => {
  try {
    ...
    if (houses.length === 0) {
      // Si aucun résultat n'est trouvé, envoyer le message d'erreur approprié
...
module.exports.removeFromFavorites = async (req, res) => {
  ...
  try {
      ...
      // Vérifier si la maison est dans les favoris de l'utilisateur
      if (!user.favorites.includes(houseId)) {
          return res.status(400).json({ message: "La maison n'est pas dans les favoris de l'utilisateur." });
      }

      // Supprimer la maison des favoris de l'utilisateur
      user.favorites = user.favorites.filter(favorite => favorite !== houseId);
      await user.save();
